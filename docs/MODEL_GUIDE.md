...
AtM discovers models through the provider API. It does not crawl the filesystem for model files and it does not currently download, import, move or delete models. For the exact current release boundary, see [Application status](../STATUS.md).
...
A provider may report capabilities such as vision, tool use, embeddings or reasoning/thinking. The current AtM release does not expose all provider/model capabilities simply because the selected model supports them.

AtM currently provides ordinary text chat plus repository-grounded text retrieval. It does not provide image input or tool-calling controls, and its default `/api/chat` request sets `think: false` to prioritize interactive latency.
...
AtM is packaged primarily for Linux/elementary OS, but the other paths are included here because they are part of the provider documentation.
...
The current AtM provider implementation connects only to loopback addresses. This means AtM talks to provider software on the same machine. It does not guarantee that every model exposed by that software performs inference locally.
