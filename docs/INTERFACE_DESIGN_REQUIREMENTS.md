...
The fixed status LCD immediately below the header provides the textual counterpart to that activity cue. It is a passive smoky-gold translucent panel with no panel glow. Only its text is luminous. The luminous text uses the same active-gold hue as the moving activity highlight, `#E6BD66`, with a very small text glow for readability. The LCD contains two logical text zones in one shared panel: local-AI/model status on the left and repository status on the right. The two zones have equal width and remain permanently allocated; status changes therefore never affect header or LCD geometry. Messages stay concise and single-line, with end ellipsization when required. The conversation transcript begins below the LCD with a small fixed separation, so the LCD occupies a narrow part of the conversation area without covering AI responses.

When no operation is running, the LCD shows persistent standby state rather than becoming blank. The left side reports the active local AI state, for example `AI · olmo-3 · connected`; if the provider is reachable but no completion-capable model is available, that limitation is stated instead. The right side reports the selected repository acronyms and readiness, for example `REPOS · EWD + RMD · ready`. If no repositories are selected it says `REPOS · none selected`; if any selected repository still needs a local validated snapshot it reports `download needed` rather than `ready`. Temporary activity/result messages replace these standby strings during checking, downloading, updating or validation, then return automatically to the current standby state. Persistent warnings such as an available update or a repository-check failure may remain until the relevant state changes.
