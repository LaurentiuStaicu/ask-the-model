...
The fixed status LCD immediately below the header provides the textual counterpart to that activity cue. It is a passive smoky-gold translucent panel with no panel glow. The LCD itself never lights up. Its annunciator text uses the same active-gold hue as the moving activity highlight, `#E6BD66`.

The LCD follows a clock/instrument-panel model rather than free-form status sentences. All supported state labels are permanently present in fixed positions. Inactive labels remain visible only as very faint, non-glowing gold segments; active labels become bright `#E6BD66` with a very small text glow. State changes therefore alter only luminance/style, never text geometry.

The LCD uses two fixed rows in one shared panel. The AI row contains `AI`, the current model field, `CONNECTED`, `SCAN`, `OFFLINE` and `ERROR`. The repository row contains `REPOS`, `EWD`, `CBD`, `RMD`, `NONE`, `CHECK`, `DOWNLOAD`, `UPDATE`, `VALIDATE`, `READY`, `OFFLINE` and `ERROR`. Repository acronyms light only when selected. `DOWNLOAD` lights when a selected repository still needs a local snapshot or while download is occurring; `UPDATE` lights when an update is available or while it is being applied. During active work, the corresponding button's orbiting activity sleeve distinguishes an operation in progress from a merely available action. `VALIDATE` lights during snapshot/index validation. `READY` means the selected local snapshots are usable and may remain lit together with `UPDATE` when a newer remote version is available, or together with `OFFLINE` when the local repositories remain usable without network access. `ERROR` lights for a failed repository operation.

The conversation transcript begins below the LCD with a small fixed separation. The panel remains permanently allocated, so neither header controls nor the transcript origin shift when status changes.
