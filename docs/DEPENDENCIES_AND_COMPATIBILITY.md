...
This document defines the compatibility contract for Ask the Model (AtM) v0.5.0.
...
AtM v0.5.0 requires an already-running local Ollama-compatible HTTP provider.
...
Providers that do not accept these fields or response semantics are not compatible with v0.5.0.

## Model requirements

A chat model must advertise the `completion` capability.

Capabilities such as `vision`, `tools`, `thinking` or `embedding` may be present, but v0.5.0 does not expose those capabilities as separate application features.
...
The v0.5.0 repository-aware release additionally requests only:
...
AtM v0.5.0 selects `GSK_RENDERER=cairo` automatically only when:
...
Only conversations explicitly saved with **Save to History** persist across restarts. Saved conversations are stored in the local conversation database and mirrored as managed JSON files under `~/Ask the Model/Conversation Exports/`; unarchived working chats are discarded on Close or normal application exit.
