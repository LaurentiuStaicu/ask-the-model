...
- completed OPT-A1-I1c2 dormant durable-ingest namespace ordering without runtime activation: durable ingest now requires a pre-existing real trusted data root, applies the shared S1 tree barrier, durably prepares the selected final-parent hierarchy, promotes by rename, fsyncs both destination and staging source parents, and returns the pre-barrier seal only after the full selected namespace sequence; all Meson/capacity standalone link dependencies and structural validators are updated, while Optimizations OFF and Vala/lifecycle callers remain unchanged;
- refined OPT-A1-P1 with the post-M10/M11 namespace policy: the selected snapshot strategy remains S1 targeted fsync and the selected namespace sequence is now `S1_DEST_SOURCE` (durable final-parent hierarchy, destination-parent fsync, source staging-parent fsync); runtime remains unwired, the dormant I1c primitive is explicitly not yet namespace-complete, and automatic orphan deletion remains unselected because I1b zero-reference evidence lacks authority-wide writer exclusion;
- added OPT-A1-F10 checked-in fresh-install namespace evidence: one strict durability-v2 registry now freezes the final M10 3×6 replay matrix and final M11 three-boundary EIO matrix with exact run/artifact/head/digest provenance; it records `S1_DEST_SOURCE` as ready for separate policy review while keeping production namespace sequencing and automatic orphan recovery explicitly unselected;
