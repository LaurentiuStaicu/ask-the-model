...
- added OPT-A1-M8 measurement-only candidate writeback-error qualification: `dm-flakey error_writes` is injected separately into S1/S2 at the pre-rename durability barrier, promoted-parent fsync and Control DB activation; each scenario requires the helper to fail at the exact injected stage and fresh-process recovery to retain the old seal-valid authority, while unsafe outcomes remain measurement evidence rather than being hidden by the harness;
