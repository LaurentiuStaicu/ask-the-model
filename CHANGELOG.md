...
- added OPT-C1-I2 non-destructive live-generation protected roots: strict read-only Control DB enumeration supplies the universe of positive COMPLETE generations, non-durable generations are probed through the existing B2 exclusive nonblocking `flock` primitive, contention adds that generation and its exact repository/SHA snapshots to the protected-root set, successful exclusive acquisition is released immediately and proves only momentary non-liveness, and lock-file existence/content is never treated as authority; candidate discovery, trash isolation, purge and lifecycle wiring remain absent;
