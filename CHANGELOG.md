...
- implemented OPT-C1-I7 bounded runtime snapshot isolation without purge: a successful user-initiated Download/Update now carries the exact operation-level Optimizations snapshot back from `RepositoryLifecycleService`; only ON operations with `changed > 0` perform at most one post-return I6 isolation attempt after the writer B0 lease is released, OFF/zero-change/lacking-conversation-store paths skip cleanup, I6 errors are diagnostic-only and cannot retroactively fail the committed repository action, no live-switch reread/background/ENOSPC/conversation-delete trigger is added, and I5 purge remains runtime-unwired;
- selected OPT-C1-P2 bounded runtime isolation-trigger policy without runtime activation: at most one dormant I6 isolation attempt may be considered only after a user-initiated Download/Update action completes successfully with a positive changed-repository count under the same operation-level Optimizations-ON snapshot and only after the writer B0 lease is released; startup, switch-toggle, idle/background, conversation close/delete/archive, repository refresh and ENOSPC/capacity-failure triggers are forbidden, purge remains separate, and any cleanup failure cannot retroactively invalidate the already-committed repository action;
- added OPT-C1-I6 dormant isolation orchestrator and race qualification: Optimizations-OFF is a no-op; ON acquires B0 exclusive, rebuilds durable+live roots, discovers one deterministic exact candidate, resolves every COMPLETE generation referencing it, acquires B2 exclusions ascending, rereads durable-only roots after exclusion, and invokes qualified I4 only if still unprotected; tests cover late shared readers, late durable roots, new-reader blocking, partial-exclusion release and owned-B2 durable reread, while no lifecycle caller, phase-2 purge or destructive runtime authorization is added;
- selected OPT-C1-P1 dormant isolation-orchestration policy: destructive isolation remains unauthorized while the required sequence is frozen as Optimizations ON -> B0 exclusive -> fresh durable/live roots -> all COMPLETE generations referencing the exact candidate -> B2 exclusive leases in ascending generation order -> durable-root reread -> exact no-follow source revalidation -> I4 isolate-to-trash; I5 purge remains a separate later phase and ENOSPC-triggered automatic GC stays unselected;
- added OPT-C1-I5 dormant isolated-trash purge primitive: canonical I4 trash identities are resolved only below `Repositories/.trash/<repo>`, the full tree is prevalidated through dirfd/no-follow traversal, recursive removal uses `unlinkat()`/`AT_REMOVEDIR` with directory fsync barriers, symlink/special-object repair conditions stop purge without any normal snapshot or runtime GC access, and process-interruption replay proves intact, resumable `TRASH_RESIDUAL`, resumable empty-root and fully-purged outcomes without adding a runtime GC caller;
- implemented OPT-C1-I3b mode-independent correctness coordination without enabling destructive GC: OFF and ON repository writers now always take the shared B0 global mutation lease, every positive repository-backed grounding always holds the B2 shared generation lease for its lifetime, mixed-mode lifecycle tests prove OFF/ON writer contention and OFF reader vs exclusive-generation exclusion, while A1/C0/B1/C1/D1 remain optimization-gated and generation 0 remains lease-free;
- selected OPT-C1-P0 authority-wide coordination prerequisite without runtime activation: G0's explicit mandatory-correctness exemption is applied only to the B0 repository mutation lease and B2 positive-generation shared lease so future destructive GC can coordinate with Optimizations-OFF as well as ON writers/readers; A1 durability, B1 index single-flight, C0 admission, C1 destructive GC and D1 remain OFF-gated, mixed-mode qualification is mandatory next, and destructive GC remains unauthorized;
- added OPT-C1-I4 dormant isolate-to-trash primitive: exact lowercase SHA snapshot directories are revalidated through dirfd/no-follow handles, moved with `renameat2(..., RENAME_NOREPLACE)` into the AtM-owned same-data-root trash namespace, and persisted with the selected destination-then-source directory fsync order; no purge or runtime GC caller is introduced;
- added OPT-C1-I3 advisory snapshot candidate discovery: only fixed-catalog repository snapshot roots are scanned through component-by-component `openat(O_DIRECTORY|O_NOFOLLOW)` descriptors, protected exact-SHA roots and only canonical `.invalid-<sha>-<timestamp>-<attempt>` quarantine directories are excluded, malformed quarantine/name/symlink/non-directory objects fail closed, intermediate namespace symlinks are rejected, and no storage mutation or runtime GC caller is introduced;
- added OPT-C1-I2 non-destructive live-generation root qualification: COMPLETE historical generations are enumerated through strict read-only Control DB metadata, B2 exclusive nonblocking probes treat only kernel-lock contention as transient liveness, uncontended leases are released immediately, and lock-file names/content remain non-authoritative;
- added OPT-C1-I1 non-destructive durable protected-root collection on top of I0: the active positive Control DB generation and every positive generation pinned by the already-open durable conversation store are deduplicated and mapped to exact repository/SHA roots using only I0 strict read-only Control DB bindings; conversation pins are cross-checked against immutable authority, malformed or contradictory metadata fails closed, and lifecycle/GC deletion remains unwired;
- added OPT-C1-I0 read-only protected-root primitives without destructive GC behavior: a strict conversation-store query lists distinct positive repository generations pinned by durable conversation rows, while strict Control DB queries read the active COMPLETE generation, read exact per-repository generation rows, and resolve any protected COMPLETE generation to exact repository/SHA roots without bootstrap/migration; missing/legacy state fails closed, while candidate discovery, trash isolation, purge and runtime GC remain absent;
- completed OPT-A1-P4 bounded runtime durability qualification: the already-wired Optimizations-ON local post-download authority path is now marked runtime-fault-qualified by composing M9 same-SHA repair, M10/M11b filesystem/namespace evidence, M12a authority interruption sequencing and M12b real-lifecycle exact-fsync EIO propagation; the completion is explicitly scoped to the selected local-filesystem contract, preserves OFF baseline behavior, makes no universal physical-power-loss or remote-filesystem claim, excludes HTTP download from the authority-preparation scope, and still does not authorize automatic orphan recovery;
- added OPT-A1-F12 frozen runtime-fault evidence: M12a process-interruption authority sequencing and M12b real-lifecycle exact-fsync EIO propagation are frozen from the same final source head with exact Actions/artifact ZIP and canonical JSON digests; M12a keeps every pre-authority interruption at generation 0 and only after-authority at one seal-valid generation, M12b keeps all four requested lifecycle fsync failures at generation 0 with no repository authority, the evidence remains non-selecting/non-power-loss, and only a separate P4 review may mark runtime fault qualification complete;
- added OPT-A1-M12b lifecycle fsync EIO qualification: an isolated test-only local-archive seam drives the real Optimizations-ON `RepositoryLifecycleService.download_or_update()` path while deterministic exact fsync failures are injected at staging-tree, fresh namespace child, promoted destination-parent and staging source-parent boundaries; every scenario fails closed with Control DB generation 0 and no repository authority, while M11b remains the separate ext4 block-layer evidence;
- added OPT-A1-M12a runtime authority sequencing qualification: the local post-download ON pipeline is interrupted at six deterministic boundaries from post-durable-ingest through after-authority; the five pre-publication boundaries remain empty valid authority, after-authority is one seal-valid new generation, and an injected promoted-snapshot seal mismatch fails closed before authority;
- implemented OPT-A1-I1d2 ON-only runtime durable ingest wiring: the frozen operation-level Optimizations snapshot now selects namespace-complete durable ingest only for ON download/update installs while OFF retains baseline ingest; the S1 pre-barrier seal is required to match the promoted pre-index seal before existing post-index integrity checks, ON rejects unqualified pre-existing final targets without reuse/removal, same-SHA repair retains quarantine then uses the durable path, and automatic orphan recovery remains disabled; end-to-end runtime fault qualification is explicitly deferred to M12;
- selected OPT-A1-P3 ON-only runtime integration policy without authorizing wiring: the operation-level Optimizations snapshot selects namespace-complete durable ingest only when ON, OFF preserves the baseline non-durable ingest path, the returned pre-barrier seal must chain through promoted/post-index verification, unqualified pre-existing final targets fail closed without reuse or removal, same-SHA repair uses durable ingest after quarantine, and automatic orphan recovery remains unselected;
- added OPT-A1-I1d1 dormant Vala durable-ingest bridge without lifecycle activation: `RepositoryNative.ingest_archive_durable()` now exposes the namespace-complete native primitive and pre-barrier seal to Vala, structural CI requires the binding while still forbidding any `RepositoryLifecycleService` call, and policy/runtime authorization remains false so Optimizations ON/OFF behavior is unchanged;
- completed OPT-A1-I1c2 dormant durable-ingest namespace ordering after F11b/P2b without runtime activation: durable ingest now requires a pre-existing real trusted data root, applies the shared S1 tree barrier, durably prepares the selected final-parent hierarchy, promotes by rename, fsyncs both destination and staging source parents, and returns the pre-barrier seal only after the full selected namespace sequence; all Meson/capacity standalone link dependencies and structural validators are updated, while Optimizations OFF and Vala/lifecycle callers remain unchanged;
- selected OPT-A1-P2b `S1_DEST_SOURCE` as the production namespace sequence after F11b exact-fsync freeze: M10 remains an empirical tie rather than a candidate ranking, while Linux containing-directory durability semantics and M11b exact hierarchy-child/containing-parent/destination-parent/source-parent fail-closed evidence justify the contract-complete sequence; runtime remains unwired, I1c is not yet namespace-complete, no syncfs fallback is added, and automatic orphan deletion remains unselected;
- added OPT-A1-F11b frozen exact namespace-fsync evidence: the final M11b run/artifact/head/digest and complete two-boundary dm-flakey plus two exact-syscall probe results are checked in and strictly validated; all measured failures remain fail-closed with empty authority, evidence and non-selecting policy staging now advance to a separate P2b namespace review, while production namespace selection, runtime wiring and automatic orphan recovery remain false;
- added OPT-A1-M11b exact namespace-fsync EIO qualification infrastructure: block-layer faults isolate the newly created repository-directory child fsync and promoted destination-parent fsync with boundary-specific error context, while deterministic test-only syscall probes isolate the containing-parent namespace fsync and post-rename source staging-parent fsync; the gate remains measurement-only, production namespace selection stays suspended, and a separate evidence-freeze/policy review is required before runtime integration;
