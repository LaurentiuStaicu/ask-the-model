...
- added OPT-C1-I3 advisory snapshot candidate discovery: only fixed-catalog repository snapshot roots are scanned through component-by-component `openat(O_DIRECTORY|O_NOFOLLOW)` descriptors, protected exact-SHA roots and real `.invalid-*` quarantine directories are excluded, malformed/symlink/non-directory objects fail closed, intermediate namespace symlinks are rejected, and no storage mutation or runtime GC caller is introduced;
