...
- selected `S1_TARGETED_FSYNC` as the A1 snapshot-authority production durability policy after the complete M1/M4/M5/M6/M8/M9 evidence review: S1 and S2 are equally qualified across the reviewed replay, interruption, EIO and same-SHA repair matrices, S2 remains the faster filesystem-wide comparator, and S1 is selected for its narrower AtM-tree synchronization/error scope; this policy-only step changes no runtime behavior, keeps S3 rejected by M4, requires later wiring behind Optimizations ON, and forbids a leftover pre-authority final snapshot from bypassing the selected barrier on retry;
