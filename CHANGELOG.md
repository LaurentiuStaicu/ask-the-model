...
- added OPT-A1-I1b native durable-ingest primitive without runtime activation: the real repository extraction/validation path can now compute the pre-barrier snapshot seal, execute the qualified shared S1 tree barrier, atomically promote the snapshot and fsync the promotion parent while returning the covered seal to a future lifecycle caller; the baseline ingest wrapper remains non-durable, existing final targets are preserved fail-closed, standalone capacity runners link the new dependencies, and structural CI forbids Vala/lifecycle activation in this slice;
