...
- completed OPT-A1-P4 bounded runtime fault-qualification policy for the Optimizations-ON local Linux/ext4 authority pipeline: frozen M12a process-interruption sequencing, M12b real-lifecycle exact-fsync EIO propagation, and the previously selected M10/M11b filesystem/barrier evidence are reviewed together; within that declared scope runtime fault qualification is complete, while physical power-loss claims, network/remote filesystem durability and automatic orphan recovery remain explicitly outside the qualification;
- completed OPT-A1-P4 bounded runtime fault-qualification review: the frozen F12 M12a process-interruption and M12b real-lifecycle fsync-EIO evidence, together with the selected M10/M11b filesystem-barrier evidence, now qualifies the Optimizations-ON local Linux/ext4 repository authority pipeline; pre-authority interruption and exact selected fsync failures remain fail-closed, the post-authority case is seal-valid, while physical-power-loss generalization, network/remote filesystem durability and automatic orphan recovery remain explicitly outside the qualified scope;
