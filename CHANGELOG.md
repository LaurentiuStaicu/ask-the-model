...
- added C0-F3 operation phase-plan qualification: explicit archive/snapshot/index/state predictions are mapped into the real download → extraction → index-build → state-commit coexistence phases, archive pre-scan materialized entries supply data-root inode demand, shared-filesystem evaluation remains delegated to C0-F1, and same-SHA repair explicitly requires admission before quarantine; no production threshold or Download/Update wiring is introduced;
