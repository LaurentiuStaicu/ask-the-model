...
- added OPT-C1-I3 non-destructive snapshot candidate discovery: a component-by-component `openat(O_DIRECTORY|O_NOFOLLOW)` scanner visits only fixed catalog repository snapshot namespaces, recognizes only lowercase 40-hex real-directory children as snapshot entries, filters all I2 protected repository/SHA roots, reports malformed names and non-directory/symlink entries as diagnostics instead of candidates, ignores arbitrary repository IDs outside the catalog, and keeps `.trash`, rename, purge and lifecycle wiring absent;
