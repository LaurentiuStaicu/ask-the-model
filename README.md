...
**Current status:** application baseline, current development version 0.1.1. The development branch now includes the native GTK 4 / Granite shell, local message composition, automatic elementary OS color-scheme following, dynamic local Ollama discovery, a refreshable selector for completion-capable models, and in-session chat through Ollama's `/api/chat` endpoint. Assistant text is streamed into the transcript as Ollama emits response chunks. The default chat path explicitly requests `think: false` for supported reasoning models so hidden reasoning does not dominate local response latency. On X11 sessions where the GTK accelerated renderers fail to initialize, AtM applies a Cairo renderer fallback before GTK starts; Wayland sessions and explicit user renderer overrides are left untouched. Repository access, conversation persistence and settings are not yet implemented. See [STATUS.md](STATUS.md) for the maintained application boundary and [CITATION.cff](CITATION.cff) for citation metadata.
...
The development Flatpak shares the network subsystem because local Ollama communication uses its HTTP API. AtM prefers a standalone local Ollama service on `127.0.0.1:11434` and retains `127.0.0.1:11435` only as a compatibility fallback. The model selector can rescan the local provider, reports real discovery progress while model capabilities are checked, and excludes embedding-only models. User prompts are sent only after the user presses **Send**, and the current in-session conversation history is included in subsequent local `/api/chat` requests.
