...
WORK_ROOT="$(mktemp -d /tmp/atm-a1-m10-XXXXXX)"
RESULTS_DIR="$WORK_ROOT/results"
mkdir -p "$RESULTS_DIR" "$(dirname "$OUTPUT")"

cleanup() {
    set +e
    for mapper in $(sudo dmsetup ls --target log-writes 2>/dev/null | awk '/atm-a1-m10-/{print $1}'); do
...
run_boundary() {
    ...
    local mapper_name="atm-a1-m10-${GITHUB_RUN_ID:-$$}-$scenario"
    ...
    "$HELPER" --initialize-empty "$original_mount" \
        >"$work/empty.json"

    test -d "$original_mount/data"
    test ! -e "$original_mount/data/Repositories"
    ...
    # Teardown occurs after the target mark. Replay stops at the mark, so
    # clean unmount/removal writes are excluded from the observation.
    ...
    "$HELPER" --verify-fresh "$replay_mount" >"$verify_json"
...
record = {
    ...
    "namespace_clean": verification.get("namespace_clean") is True,
}
...
strategies=(
    "S1_PARENT_ONLY:parent_only"
    "S1_DEST_CHAIN:dest_chain"
    "S1_DEST_SOURCE:dest_source"
)

boundaries=(
    "fresh_post_barrier_pre_destination_hierarchy:post_barrier_pre_destination_hierarchy:EMPTY_AUTHORITY_VALID"
    "fresh_post_destination_hierarchy_pre_rename:post_destination_hierarchy_pre_rename:EMPTY_AUTHORITY_VALID"
    "fresh_post_rename_pre_destination_parent_fsync:post_rename_pre_destination_parent_fsync:EMPTY_AUTHORITY_VALID"
    "fresh_post_destination_parent_fsync_pre_source_parent_fsync:post_destination_parent_fsync_pre_source_parent_fsync:EMPTY_AUTHORITY_VALID"
    "fresh_post_source_parent_fsync_pre_authority:post_source_parent_fsync_pre_authority:EMPTY_AUTHORITY_VALID"
    "fresh_after_authority:after_authority:NEW_AUTHORITY_VALID"
)

for strategy_record in "${strategies[@]}"; do
    strategy="${strategy_record%%:*}"
    slug="${strategy_record##*:}"

    for boundary_record in "${boundaries[@]}"; do
        checkpoint="${boundary_record%%:*}"
        rest="${boundary_record#*:}"
        boundary_slug="${rest%%:*}"
        expected="${rest##*:}"
...
if {item["strategy"] for item in results} != strategies:
    raise SystemExit("A1-M10 strategy set is incomplete")
if {item["boundary"] for item in results} != boundaries:
    raise SystemExit("A1-M10 boundary set is incomplete")
if len(results) != len(strategies) * len(boundaries):
    raise SystemExit("A1-M10 must contain exactly 18 scenarios")

summaries = {}
for strategy in sorted(strategies):
    subset = [item for item in results if item["strategy"] == strategy]
    by_boundary = {item["boundary"]: item for item in subset}

    if set(by_boundary) != boundaries:
    ...
    post_source = by_boundary[
        "post_source_parent_fsync_pre_authority"
    ]

    summaries[strategy] = {
        "all_pre_authority_empty_valid": all(
            item["authority_target_satisfied"]
            for item in pre
        ),
        "after_authority_new_valid": (
            after["authority_target_satisfied"]
        ),
        "after_authority_namespace_clean": (
            after["namespace_clean"]
        ),
        "post_source_boundary_namespace_clean": (
            post_source["namespace_clean"]
        ),
        "all_e2fsck_recoverable": all(
            item["e2fsck_exit_code"] in {0, 1}
            for item in subset
        ),
    }
...
artifact = {
    "schema_version": 1,
    "measurement_id": "atm-a1-m10-fresh-namespace-replay-v1",
    ...
    "protocol": {
        "fresh_control_db_without_repository_authority": True,
        "repositories_hierarchy_absent_at_baseline": True,
        "baseline_synced_before_candidate": True,
    ...
    "limitations": [
        (
            "M10 is fresh-install namespace replay qualification and "
            "does not activate application runtime durability."
        ),
        (
            "S1_PARENT_ONLY represents the currently merged but "
            "runtime-unwired I1c primitive shape."
        ),
        (
            "M11 separately qualifies explicit EIO at any newly "
            "selected namespace fsync barriers."
        ),
        (
            "Linux requires explicit containing-directory fsync for "
            "directory-entry persistence; replay evidence does not "
            "weaken that platform contract."
        ),
    ],
}
