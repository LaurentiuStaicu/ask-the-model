...
require_text STATUS.md 'The development GTK application now contains the draft EWD/CBD/RMD selector and repository lifecycle controls, while the message-generation path still uses ordinary local chat and does not yet expose end-to-end grounded chat or user-visible citation rendering.'
