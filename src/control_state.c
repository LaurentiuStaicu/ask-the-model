...
)
{
    if (!nonempty (path) ||
        ...
        g_set_error_literal (
            ...
            ATM_CONTROL_STATE_ERROR,
            ATM_CONTROL_STATE_ERROR_ARGUMENT,
            "Control-state COMPLETE-generation listing received invalid arguments."
        );
        return FALSE;
    }
    ...
    *out_count = 0;
    ...
    while ((rc = sqlite3_step (statement)) ==
        ...
        if (generation_ids->len >= G_MAXINT) {
            g_set_error_literal (
                error,
                ATM_CONTROL_STATE_ERROR,
                ATM_CONTROL_STATE_ERROR_INTEGRITY,
                "COMPLETE repository generation listing exceeds the supported array size."
            );
            goto done;
        }
    ...
    *out_count = (gint) generation_ids->len;
